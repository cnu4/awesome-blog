...
def authenticate():
	...
	if user is None:
		...
		raise APIError('auth:failed', 'password', 'Incorrect password')
	# make session cookie
...
def register_user():
	...
	email = i.email.strip().lower()
	password = i.password
	logging.info('api-name: %s email: %s password: %s' % (name, email, password))
	if not name:
	...
	user = User(name=name, email=email, password=password, image='https://en.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email).hexdigest())
	user.insert()
	# make session cookie:
