...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
	...
	if engine is not None:
		raise DBError('Engine is already initialize.')
	...
	logging.info('Init mysql engine <%s> ok. host: %s port: %s' % (hex(id(engine)), host, port))
	'''
	import MySQLdb
	global engine
	if engine is not None:
...
def _select(sql, first, *args):
	'execute select SQL and return result or lsit result'
	global _db_ctx
	cursor = None
	sql = sql.replace('?', '%s')
...
def select(sql, *args):
	'''
	Execute select SQL and return list or empty list.
